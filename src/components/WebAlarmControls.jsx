...
function WebAlarmControls({ snapshot }) {
  ...
  const editAlarm = alarm => { setEditingId(alarm.id); setEditingDraft({ metric: alarm.metric, condition: alarm.condition, threshold: String(alarm.threshold), repeatSeconds: alarm.repeatSeconds }); };
  const saveEditedAlarm = () => { const value = Number(editingDraft?.threshold); if (!editingId || !Number.isFinite(value)) return; update(editingId, { metric: editingDraft.metric, condition: editingDraft.condition, threshold: value, repeatSeconds: editingDraft.repeatSeconds }); setEditingId(null); setEditingDraft(null); };
