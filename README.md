...
The **Metric Trends** graph at the bottom samples Heart Rate, RMSSD, SDNN, and BRPM every five seconds. Its checkboxes let you show or hide each series. Since these measurements use incompatible units and ranges, every line is positioned relative to its own observed range; the raw latest value, unit, and range remain listed below the graph. This makes within-metric trends comparable without implying that equal chart heights mean equal physiological magnitude.
